window.onload = function () {
  ...
  const width_container = document.querySelector(
    ".witeCar__container"
  ).clientWidth;
  let html_page = 0;
  let width = 0;
...
iconmenu.addEventListener("click", (e) => {
  if (!menu.classList.contains("menu__body_active")) {
    ...
    setTimeout(function () {
      user.classList.add("rotateMyUser");
    }, 600);
  } else {
...
for (let i = 0; i < subclass.length; i++) {
  subclass[i].addEventListener("click", (e) => {
    ...
    if (
      e.target.dataset.subletter === "b" ||
      e.target.dataset.subletter === "c" ||
      e.target.dataset.subletter === "d" ||
      e.target.dataset.subletter === "j"
    ) {
...
let subtransCURR = 0;
document
  .querySelector(".subtranss__absolute")
...
for (let i = 0; i < subtrans.length; i++) {
  subtrans[i].addEventListener("click", (e) => {
    ...
    console.log(iusefilter);
    if (
      e.target.dataset.subtransmission === "auto" ||
      e.target.dataset.subtransmission === "mechanic"
    ) {
...
for (let i = 0; i < substyle.length; i++) {
  substyle[i].addEventListener("click", (e) => {
    ...
    console.log(iusefilter);
    if (
      e.target.dataset.subclass === "suv" ||
      e.target.dataset.subclass === "minivan" ||
      e.target.dataset.subclass === "pic" ||
      e.target.dataset.subclass === "sed" ||
      e.target.dataset.subclass === "univ"
    ) {
      ...
      if (input_value == "бмв" || input_value == "bmw") {
        ...
        if (input_value == "фиат" || input_value == "fiat") {
          ...
          if (input_value == "фольксваген" || input_value == "volkswagen") {
            ...
            if (
              input_value == "мильтсубиси" ||
              input_value == "mitsubishi" ||
              input_value == "митсубиси"
            ) {
              input_value = "mitsub";
            } else {
...
function hider() {
  ...
  if (check_morebtn == false) {
    for (let i = 0; i < before_click.length; i++) {
      ...
      setTimeout(function () {
        before_click[i].style.display = "none";
      }, 400);
    }
  } else {
    for (let i = 0; i < before_click.length; i++) {
    ...
    for (let i = 0; i < after_click.length; i++) {
      ...
      setTimeout(function () {
        after_click[i].style.display = "none";
      }, 400);
    }
  }
}
